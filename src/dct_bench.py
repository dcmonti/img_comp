...
time_complexity3 = [(i**3)/10**9 for i in sizes]

plt.plot(sizes, scipy_times, label='scipy dct')
plt.plot(sizes, libc_times, label='libc dct')
plt.plot(sizes, my_times, label='my dct')
plt.plot(sizes, time_complexity2, label=r'$size^2*log_2(size)$', linestyle='dashed')
plt.plot(sizes, time_complexity3, label=r'$size^3$', linestyle='dashed')
